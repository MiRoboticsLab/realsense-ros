...
void AlignRealSenseNode::setAvailableSensors()
{
    ...
    for(auto&& sensor : _dev_sensors)
    {
        ...
        else if (sensor.is<rs2::motion_sensor>())
        {
            ...
            rosSensor = std::make_unique<RosSensor>(sensor, _parameters, imu_callback_function, update_sensor_func, hardware_reset_func, _diagnostics_updater, _logger, false);
        }
        else if (sensor.is<rs2::pose_sensor>())
        {
            ROS_DEBUG_STREAM("Set " << module_name << " as PoseSensor.");
            rosSensor = std::make_unique<RosSensor>(sensor, _parameters, multiple_message_callback_function, update_sensor_func, hardware_reset_func, _diagnostics_updater, _logger, false);
        }
        else
        {
...
void AlignRealSenseNode::setCallbackFunctions()
{
...
void AlignRealSenseNode::stopPublishers(const std::vector<stream_profile>& profiles)
{
...
void AlignRealSenseNode::startPublishers(const std::vector<stream_profile>& profiles, const RosSensor& sensor)
{
    ...
    for (auto& profile : profiles)
    {
        ...
        rmw_qos_profile_t publish_alinged_depth_qos =
        {
        ...
        if (profile.is<rs2::video_stream_profile>())
        {
            ...
            {
                const auto stream_name = std::string{"extcolor"};
                auto sip = COLOR;
#else
            if (_align_depth_filter->is_enabled() && (sip != DEPTH) && sip.second < 2)
            {
                ...
                if (_use_intra_process)
                {
                    _depth_aligned_image_publishers[sip] = std::make_shared<image_rcl_publisher>(_node, aligned_image_raw.str(), publish_alinged_depth_qos);
                }
                else
                {
                    _depth_aligned_image_publishers[sip] = std::make_shared<image_transport_publisher>(_node, aligned_image_raw.str(), publish_alinged_depth_qos);
                    ROS_DEBUG_STREAM("image transport publisher was created for topic" << image_raw.str());
                }
...
void AlignRealSenseNode::updateSensors()
{    
...
void AlignRealSenseNode::publishServices()
{
...
void AlignRealSenseNode::getDeviceInfo(const realsense2_camera_msgs::srv::DeviceInfo::Request::SharedPtr,
                                            realsense2_camera_msgs::srv::DeviceInfo::Response::SharedPtr res)
{
