...
AlignNodeFactory::AlignNodeFactory(const rclcpp::NodeOptions & node_options) :
	nav2_util::LifecycleNode("camera_align","/camera",false,node_options),
...
AlignNodeFactory::AlignNodeFactory(const std::string & node_name, const std::string & ns,
										   const rclcpp::NodeOptions & node_options) : 
	LifecycleNode(node_name,ns,false,node_options),
	_logger(this->get_logger()),
	_init(false),
	_state(false)
{
...
AlignNodeFactory::~AlignNodeFactory()
{
	_is_alive = false;
...
std::string AlignNodeFactory::parseUsbPort(std::string line)
{
    std::string port_id;
...
void AlignNodeFactory::getDevice(rs2::device_list list)
{
	if (!_device)
	{
		if (0 == list.size())
...
void AlignNodeFactory::changeDeviceCallback(rs2::event_information& info)
{
	if(!_activate)
		return;
	if (info.was_removed(_device))
	{
		...
		{
			ROS_INFO_STREAM("wait for the device recovery timeout !");
		}
		...
		{
			_recovery = result.get()->success;
			if(_recovery)
				ROS_INFO_STREAM("The device has been recovery");
			else
				ROS_INFO_STREAM("The device  recovery false");
		}
	}
	if (!_device)
	{
		...
		if (new_devices.size() > 0)
		{
			...
			if (_device)
			{
				ROS_INFO("RealSense is OnActivate!");
				startDevice();
				_recovery = false;	
			}
		}
...
std::string AlignNodeFactory::api_version_to_string(int version)
{
...
void AlignNodeFactory::init()
{
	try
	{
		...
		{
			_is_alive = true;
			_parameters = std::make_shared<Parameters>(*_node);
...
void AlignNodeFactory::startDevice()
{
	...
	try
	{
		...
		case RS_L515_PID:
		case RS_L535_PID:
			_realSenseNode = std::unique_ptr<AlignRealSenseNode>(new AlignRealSenseNode(*_node, _device, _parameters, this->get_node_options().use_intra_process_comms()));
			break;
		default:
			ROS_FATAL_STREAM("Unsupported device!" << " Product ID: 0x" << pid_str);
		}
		_state = _realSenseNode->publishTopics();
...
void AlignNodeFactory::tryGetLogSeverity(rs2_log_severity& severity) const
{
...
nav2_util::CallbackReturn AlignNodeFactory::on_configure(const rclcpp_lifecycle::State& /*state*/)
{
	_activate = false;
	if(!_init)
	{
		_node = std::make_shared<rclcpp::Node> ("realsense2_align_node","/camera",this->get_node_options());
		_client = _node->create_client<std_srvs::srv::Trigger>("realsense_recovery");
	}
	return nav2_util::CallbackReturn::SUCCESS;
}

nav2_util::CallbackReturn AlignNodeFactory::on_activate(const rclcpp_lifecycle::State& /*state*/)
{
	_activate = true;
	if(!_init)
	{
	...
	else
	{
		if(!_device)
		{
			ROS_INFO_STREAM("RealSense need to get new device");
			getDevice(_ctx.query_devices());
			if(_device)
			{
				init();
				...
				ROS_ERROR_STREAM("get device false");
		}
	}
	while(!_state)
	{
	...
	ROS_INFO_STREAM("Sending synchronous signal ...");
	sleep(1);
	ROS_INFO_STREAM("Synchronous signal has been sent");
	createBond();
	return nav2_util::CallbackReturn::SUCCESS;
}

nav2_util::CallbackReturn AlignNodeFactory::on_deactivate(const rclcpp_lifecycle::State& /*state*/)
{
	_activate = false;
	_realSenseNode.reset(nullptr);
	_device.hardware_reset();
	_device = rs2::device();
	_state = false;
	ROS_INFO_STREAM("RealSense device has been reset");
	destroyBond();
	return nav2_util::CallbackReturn::SUCCESS;
}

nav2_util::CallbackReturn AlignNodeFactory::on_cleanup(const rclcpp_lifecycle::State& /*state*/)
{
	_activate = false;
	return nav2_util::CallbackReturn::SUCCESS;
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(realsense2_camera::AlignNodeFactory)
