...
RealSenseNodeFactory::RealSenseNodeFactory(const rclcpp::NodeOptions & node_options) :
	nav2_util::LifecycleNode("camera","/camera",false,node_options),
					...
					while (_is_alive && !_device)
					{
						try
						{
							...
							if (_device)
							{
								...
								ROS_INFO_STREAM("set_devices_changed_callback");
								// startDevice();
							}
							else
							{
...
void RealSenseNodeFactory::startDevice()
{
	...
	try
	{
		...
		case RS_L515_PID:
		case RS_L535_PID:
			_realSenseNode = std::unique_ptr<BaseRealSenseNode>(new BaseRealSenseNode(*_node, _device, _parameters, this->get_node_options().use_intra_process_comms()));
			break;
		case RS_T265_PID:
			_realSenseNode = std::unique_ptr<T265RealsenseNode>(new T265RealsenseNode(*_node, _device, _parameters, this->get_node_options().use_intra_process_comms()));
			break;
		default:
...
nav2_util::CallbackReturn RealSenseNodeFactory::on_configure(const rclcpp_lifecycle::State& /*state*/)
{
	_activate = false;
	if(!_init)
	{
		_node = std::make_shared<rclcpp::Node> ("realsense2_camera_node","/camera",this->get_node_options());
		_init = true;
...
nav2_util::CallbackReturn RealSenseNodeFactory::on_activate(const rclcpp_lifecycle::State& /*state*/)
{
	_activate = true;
	_parameters = std::make_shared<Parameters>(*_node);
	getDevice(_ctx.query_devices());
...
nav2_util::CallbackReturn RealSenseNodeFactory::on_deactivate(const rclcpp_lifecycle::State& /*state*/)
{
	_activate = false;
	_realSenseNode.reset(nullptr);
	_device.hardware_reset();
...
nav2_util::CallbackReturn RealSenseNodeFactory::on_cleanup(const rclcpp_lifecycle::State& /*state*/)
{
	_activate = false;
	return nav2_util::CallbackReturn::SUCCESS;
}
