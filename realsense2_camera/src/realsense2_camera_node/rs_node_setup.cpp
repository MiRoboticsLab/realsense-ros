    ...
    for (auto& profile : profiles)
    {
        ...
        if (profile.is<rs2::video_stream_profile>())
        {
            ...
            if ((sip != DEPTH) && sip.second < 2)
            {
                ...
                ROS_DEBUG("_use_intra_process------------------%d",_use_intra_process);
                if (_use_intra_process)
                {
            ...
            if(sip == DEPTH)
            {
                std::stringstream aligned_image_raw;
                std::string align_name = "rgb";
                aligned_image_raw << "aligned_depth_to_" << align_name << "/image_raw";
                //aligned_camera_info << "aligned_depth_to_" << align_name << "/camera_info";

                std::string aligned_stream_name = "aligned_depth_to_" + align_name;
                if (_use_intra_process)
                {
                    _depth_aligned_rgb_publisher = std::make_shared<image_rcl_publisher>(_node, aligned_image_raw.str(), qos);
                }
                else
                {
                    _depth_aligned_rgb_publisher = std::make_shared<image_transport_publisher>(_node, aligned_image_raw.str(), qos);
                    //ROS_DEBUG_STREAM("image transport publisher was created for topic" << image_raw.str());
                }                
            }
        }
