...
"""Launch align_realsense2_camera node."""
import os
...
configurable_parameters = [{'name': 'camera_name',                  'default': 'camera', 'description': 'camera unique name'},
                           {'name': 'align_config_file_path',       'default': '/params/camera/calibration', 'description': 'camera unique name'},
                           ...
                           {'name': 'depth_module.inter_cam_sync_mode', 'default': '1', 'description': 'sync camera'},
                           {'name': 'wait_for_device_timeout',      'default': '-1.', 'description': 'Timeout for waiting for device to connect (Seconds)'},
                           {'name': 'reconnect_timeout',            'default': '6.', 'description': 'Timeout(seconds) between consequtive reconnection attempts'},
                           {'name': 'depth_module.emitter_enabled',         'default': '1', 'description': 'emitter_on_off'},
    ...
    if (os.getenv('ROS_DISTRO') == "dashing") or (os.getenv('ROS_DISTRO') == "eloquent"):
        return LaunchDescription(declare_configurable_parameters(configurable_parameters) + [
            ...
            launch_ros.actions.Node(
                ...
                node_executable='realsense_align_node',
                prefix=['stdbuf -o L'],
        ...
        return LaunchDescription(declare_configurable_parameters(configurable_parameters) + [
            ...
            launch_ros.actions.Node(
                ...
                executable='realsense_align_node',
                parameters=[set_configurable_parameters(configurable_parameters)
                            ],
                output='screen',
                arguments=['--ros-args', '--log-level', LaunchConfiguration('log_level')],
